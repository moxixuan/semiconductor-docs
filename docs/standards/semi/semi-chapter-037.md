...
segment.
10. 3.4.2    Internal   Change   Requests   ó   A   change
...
segment-initiated.
10. 3.4.3    Allowable   Change   Requests   ó   A   change
...
ó A request to create a new recipe, or copy an existing
...
ó A request from a DRNS segment to rename a recipe
shall  be  granted  only  if  no  other  segment  has  that
recipe and the new identifier is not otherwise in use.
ó  A  request  from  a  dedicated  segment  to  delete  a
recipe  shall  be  denied  only  if  the  recipe  is  write-
protected.
10. 3.4.4    Change   Request   Record   Definition   ó   A
change   request   record,   as   shown   in   Figure   5.7,
consists of the following information:
ó The recipe identifier (RecipeID
),
ó a destination  recipe  identifier (DestRecipeID
) (used
for copy and rename only, and otherwise null),
ó  the  segment  specifier,  (Segment
),  of  the  DRNS
segment that has requested a change or to which the
change operation is or will be delegated,
ó the specific type of change requested (ChangeType
),
ó  a  timestamp  of  when  the  request  was  received
(Timestamp
),
ó  an  operation  identifier  (OperationID
),  an  integer
...
10. 3.4.5    Change   Request   Lifecycle   ó   This   section
...
approves the segmentís earlier request to change generic
attributes:
...
10. 3.4.6   Change  Request  Completion  ó  The  DRNS
...
10. 3.4.7    Change   Management   Example   ó   As   an
...
10. 3.5    Segment   Change   Request   ó   The   segment
...
segment.
10. 3.6   Segment  Action  Complete  ó  A  DRNS  segment
...
segment.
10. 3.7  Segment  Notification  ó  A  dedicated  segment
...
10. 3.8  Get  Change  Requests  ó  The  get  change  requests  operation  returns  a  list  of  active  and  inactive change
...
10. 3.9  Rebuild Distributed Recipe Namespace ó The rebuild distributed recipe namespace operation is used to
...
10. 4   Tables  of  Operations  ó  This  section  provides  the  operations  defined  for  each  object  within  the  distributed
recipe namespace capability. An additional column for the object authorized to invoke the operation is provided for
tables in this section.
10. 4.1  Segment Operations Table ó Table 10.1 lists all the operations defined for DRNS segment.
...
10. 4.2  Recorder  Operations  Table  ó  Table  10.2  lists  all  the  operations  defined  for  distributed  recipe  namespace
recorder.

Table 10.2  Distributed Recipe Namespace Recorder Operations
Operation                                           Invoked                                           by                                           Description                                           Reqd
add segment record the recorderís manager A segment specifier is added to the
current list.
Y
delete segment record the recorderís manager A segment specifieris removed from the
current list.
Y
add change request record the recorderís manager A currently active change request
record is added for a specified recipe.
Y
delete change request record the recorderís manager An existing change request record is
...
10. 4.3  Manager  Operations  Table  ó  Table  10.3  lists  all  the  operations  defined  for  distributed  recipe  namespace
manager.
...
recipe.
Y
segment change complete an attached segment A segmentís notification that an
approved change has been completed,
either normally or abnormally.
Y
segment notification an attached dedicated segment A dedicated segmentís notification that
...
11. 1   Object  Services  Operations  ó  A  recipe  executor
...
11. 1.1    Execution   Recipe   Specifier   ó   The   object
...
11. 2  Description    of    Operations    ó    This    section
describes the operations of the recipe executor.
11. 2.1   Recipe  Download  and  Verify  ó  A  recipe  is
...
11. 2.2  Recipe  Verify  ó  This  is  a  required  operation
...
seconds,  used  for  the  EstRunTime
attribute  of  the
...
Section 11.2.1.
11. 2.2.1     Derived    Object    Form    Recipes    ó    For
...
11. 2.2.2  Verification ID       ó       The       attribute
VerificationID
...
11. 2.3  Recipe   Upload   ó   Recipe   upload   is   the
...
RMEUpload service.
11. 2.4  Recipe    Rename    ó    The    recipe    rename
...
service RMERename.
11. 2.5   Get  Available  Storage  ó  The  get  available
...
11. 2.6  Recipe  Delete  ó  The  recipe  delete  operation
...
11. 2.7  Recipe  Selection   ó   Recipe   selection   is   the
...
RMESelect service.
11. 2.7.1  Multiple  Selection  ó  Executing  agents  that
...
11. 2.7.2  Validation ó Validation consists of type and
...
the same time.
11. 2.7.3  Delegation    ó    If    delegated    recipes    are
...
11. 2.7.4  Variable   Parameters   ó   For   recipes   with
...
restriction  is  given  in  the  ExecLinkParam
attribute,
then  that  restriction  is  used  for  all  occurrences  of  the
parameter.
11. 2.7.5  Message Scenarios  ó  There  are  two  possible
...
11. 2.7.6  Selected  Recipes  ó  Once  a  recipe  and  its
...
execution.
11. 2.7.6.1  Select  Errors  ó  If  the  recipe  executor  is
...
The select   recipe   operation   is   invoked   with   the
RMESelect service.
11. 2.8  Recipe   Deselection   ó   A   selected   recipe   is
...
RMEDeselect service.
11. 2.9   Get  Execution  Recipe  Descriptor  ó  The  get
...
11. 2.10  Change  Control  ó  It  is  important  to  the  user
...
attribute
11. 2.10.1  Changing Existing Recipes ó Whenever the
...
ChangedBody
attribute is reset when the upload request
...
11. 2.10.2   Creating  New  Recipes  ó  A  new  execution
...
been linked.
11. 2.10.3  Building  Derived  Form  Recipes  ó  A  new
...
recipe.
11. 2.10.4  Saving Last Value ó Saving the most recent
...
ExecLength
and    ExecChgTime    shall    be    updated
...
11. 2.10.5  Change  Notification  ó  If  ExecChgCtrl  requires  that  the  originating  namespace  be  notified  of  changes,
...
protected recipe.
11. 3  Table of Operations ó Table 11.1 lists all the operations defined for recipes and namespaces.
...
N
Recipe rename Change the recipeís identifier. Y
...
11. 4  Recipe Executor Events ó The recipe executor shall provide the Recipe Select and Recipe Deselect events.
...
RMNVarPar                  N                  Modify                  agent-specific variable
...
Dictionary  ó  Table  12.2  contains  the  definitions  for
...
12. 2  Message Flow ó The flow of messages defined in RMS, between the service user and the service provider, are
...
The  second  type  of  message  flow  may  be  used  for  notifying  the  service  user  of  RMNAction  that  individual
operations on recipes have been completed. The first type of message flow is used for all other namespace services.
12. 3  RMNCreateNS ó The service user may request to create a new namespace  and  assign  it  a  name  (identifier).
...
operation.
12. 4  RMNDeleteNS ó The service user may request to
...
result of the requested
operation.
12. 5  RMNRenameNS  ó  The  service  user  may  request
...
result of the requested
operation.
12. 6  RMNSpaceInquire   ó   The   service   user   may
...
result of the requested
operation.
12. 7  RMNRcpStatInquire   ó The   service   user   may
...
result of the requested
operation.
12. 8  RMNVersionInquire   ó The   service   user   may
...
result of the requested
operation.
12. 9  RMNCreate  ó  A  service  user  may  request  to
...
12. 10  RMNUpdate  ó  The  service  user  requests  to
...
12. 11  RMNStore  ó The  service  user  may  request  to
...
result of the requested
operation.
12. 12  RMNRetrieve ó The service user may request to
...
the result of the
requested operation.
12. 13  RMNCopy  ó  The  service  user  may  request  to
...
12. 14  RMNRename ó The service user may request to
...
RMDestRcpIDM - The identifier for the recipe
to be originated (destination).
NSStatus - M Information concerning the
result of the requested
operation.
12. 15  RMNAction ó RMNAction is used to request the
...
NSStatus - M Information concerning the
result of the requested operation.
12. 16  RMNVarPar ó The service user may request to
...
NSStatus - M Information concerning the
result of the requested operation.
12. 17   RMNGetDescriptor  ó The  service  user  may
...
NSStatus - M Information concerning the
result of the requested operation.
12. 18  RMNComplete  ó The  service  provider  notifies
...
manager. (OSS service)
RMDSApproveAction                     R                     A                     segmentís manager approves an earlier change request.
...
13. 1   Distributed  Recipe  Namespace  Message  Parameter  Dictionary ó  This  section  defines,  in  alphabetical  order,
...
13. 2.1  RMDSApproveAction  ó  A  distributed  recipe  namespace  manager  may  approve  or  deny  a  change  request
...
13. 3.1  RMDRAddSegRecord ó A recorderís manager may request the recorder to add a segment record.
...
Attach Token M - The recorderís attach token.
NSStatus - M Information concerning the result of the requested operation.
13. 3.2  RMDRDelSegRecord ó A recorderís manager may request the recorder to delete a segment record.
...
Attach Token M - The recorderís attach token.
NSStatus - M Information concerning the result of the requested operation.
13. 3.3  RMDRAddChgRecord ó A recorderís manager may request the recorder to add a change request record.
...
13. 3.4  RMDRDelChgRecord ó A recorderís manager may request the recorder to delete a change request record.
...
Attach Token M - The recorderís attach token.
NSStatus - M Information concerning the result of the requested operation.
13. 3.5  RMDRGetChgRecord  ó  A  service  user  may  request  an  attached  recorder  to  return  the  current  change
...
13. 4.1  RMDComplete ó An attached segment notifies its manager when it has completed an approved change to a
recipe.
...
13. 4.2  RMDNotify  ó  An  attached  dedicated  segment  notifies  its  manager  that  it  has  changed  an  agent-specific
...
NSStatus - C Information concerning the result of the requested operation.
13. 4.3  RMDSegChange ó An attached segment may request its managerís approval to make a change. The recipe
specifier RMRcpSpec shall include the requesting segmentís object type and name to identify the requestor and the
...
13. 4.4  RMDGetChangeRequests  ó  A  service  user  may  request  that  all  existing  change  requests  for  a  recipe  be
...
NSStatus - M Information concerning the result of the requested operation.
13. 4.5  RMDRebuild  ó  A  service  user  may  request  a  distributed  namespace  manager  to  rebuild  a  distributed
namespace.
...
RMERename                                     R                                     Change                                     a                                     recipeís                                     identifier.
...
14. 1   Recipe  Executor  Message  Parameter  Dictionary  ó  For  purposes  of  transferring  a  recipe,  it  is  regarded  as
having two types of sections: an attribute section containing the recipeís attributes, and a body section consisting of
...
a. A recipe specifier may reference a recipe that is in a namespace and not in the recipe executorís storage.

14. 2  Message  Flow  ó  The  flow  of  messages  defined  in  RMS,  between  the  service  user  and  the  service  provider,
...
14. 3  RMEDnldVer ó The service user may request the recipe executor to download and verify an execution recipe.
...
14. 4  RMEVerify  ó  The  service  user  may  request  the  recipe  executor  to  verify  an  execution  recipe.  If  the
...
14. 5  RMEUpload ó RMEUpload retrieves a recipe from local storage and sends it to the service user.
...
14. 6  RMERename ó RMERename changes the identifier of an execution recipe through changing one or more of
...
14. 7  RMESpaceInquire ó The service user may request the amount of space available for recipes. The parameters
...
14. 8  RMEDelete ó RMEDelete removes one or more recipes from local storage and frees any reusable storage area
...
14. 9  RMESelect ó The service user may specify one or more recipes to be selected for execution. RMESelect is the
...
14. 10  RMEDeselect ó The service user may deselect a recipe to prevent its re-execution.
...
14. 11  RMEGetDescriptor ó The service user may request the descriptors of one or more recipes.
...
14. 12  RMEChange  ó  The  service  provider  informs  an  originating  namespace  that  a  permitted  change  has
occurred.
...
14. 13  RMEComplete  ó  The  service  provider  notifies  the  service  user  that  an  action  requested  by  the  service  user
has completed.
Parameters for RMEComplete are listed in Table 14.13.



SEMI E42-0704 © SEMI 1995, 2004 101
...
15. 1  Areas of Compliance ó There are seven areas of RMS compliance, as follows:
a. Managed Recipe
...
f. Execution Recipe
g. Recipe Executor
15. 2  Managed Recipe ó This section defines RMS compliance for the managed recipe.
...
SEMI E42-0704 © SEMI 1995, 2004 102
...
15. 3  Managed Recipe Compliance Table ó Table 15.2 provides a statement of compliance for the managed recipe.
...
15. 4  Recipe Namespace Management ó Recipe Namespace Management requires compliance to specifications for
the managed recipe, the recipe namespace, and the recipe namespace manager.
15. 4.1  Recipe  Namespace  ó  This  section  defines  RMS  compliance  for  the  recipe  namespace  object.  A  recipe
namespace exhibits RMS compliance through its attributes and access to managed recipes.



SEMI E42-0704 © SEMI 1995, 2004 103
...
Capability                                                                          Reference                                                                          Fundamental
Fundamental compliance to managed recipe 15.2, Table 15.2 Y
Basic requirements 4.3, 4.4, 4.6, 8.2.1 Y

15. 4.2  Recipe Namespace Manager ó Table 15.4 describes RMS compliance for the recipe namespace manager.
Table 15.4  Section References for Recipe Namespace Manager Capabilities
Capability                                                                          Reference                                                                          Fundamental
Basic requirements 4.3, 4.6 (Table 4.2), 8.2.1 Y
OSS services 9.1, 9.1.1ñ9.1.2 Y
...
Download execution recipe (RMNAction) 6.3.2, 9.4.8, 12.15 Y
...
Upload execution recipe (RMNAction) 9.4.9, 12.15 N
...
SEMI E42-0704 © SEMI 1995, 2004 104
15. 4.3  Recipe  Namespace  Management  Compliance  Table  ó  Table  15.5  provides  a  statement  of  compliance  for
recipe namespace management.
Table 15.5  Recipe Namespace Management Compliance Table
RMS Recipe Namespace Management                        Compliance Statement
Fundamental RMS Requirements Implemented RMS Compliant
Managed Recipe Object ❒ Yes  ❒ No
Recipe Namespace Object ❒ Yes  ❒ No
Recipe Namespace Manager Object Basic
Requirements
❒ Yes  ❒ No
OSS Services ❒ Yes  ❒ No
Create Recipe ❒ Yes  ❒ No
Update Recipe ❒ Yes  ❒ No
Verify Recipe ❒ Yes  ❒ No
Link Recipe ❒ Yes  ❒ No
Get Recipe Descriptor ❒ Yes  ❒ No
Get Available Space ❒ Yes  ❒ No
Check Recipe Status ❒ Yes  ❒ No
Get Best Version ❒ Yes  ❒ No
Store Recipe ❒ Yes  ❒ No
Download Recipe ❒ Yes  ❒ No
Retrieve Recipe ❒ Yes  ❒ No
Recipe Namespace Change Event ❒ Yes  ❒ No
...
Upload Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Unlink Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Approve Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Protect Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Unprotect Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Certify Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Decertify Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Delete Namespace ❒ Yes  ❒ No ❒ Yes  ❒ No
Rename Namespace ❒ Yes  ❒ No ❒ Yes  ❒ No
Copy Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Upload Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Modify Recipe Variable Parameters ❒ Yes  ❒ No ❒ Yes  ❒ No
Recipe Synchronization ❒ Yes  ❒ No ❒ Yes  ❒ No
3
Do not mark YES unless all fundamental RMS requirements are implemented.
4
Additional capabilities may not be marked RMS compliant unless all fundamental RMS requirements are implemented.
...
SEMI E42-0704 © SEMI 1995, 2004 105
15. 5    Compliance   for   Distributed   Recipe   Namespace   Management   ó   Compliance   for   distributed   recipe
management requires compliance to the following objects:
• managed recipe
• recipe namespace
• distributed recipe namespace
• recipe namespace manager
• distributed recipe namespace manager
15. 5.1  Distributed Recipe Namespace ó Table 15.6 describes the requirements for compliance for the distributed
recipe namespace.
...
Rebuild distributed recipe namespace (RMDRebuild) 5.10, 10.3.9, 13.4.5 Y




SEMI E42-0704 © SEMI 1995, 2004 106
15. 5.3  Distributed  Recipe  Namespace  Management  Compliance  Table  ó  Table  15.8  provides  a  statement  of
compliance for distributed recipe namespace management.
Table 15.8 Distributed Recipe Namespace Management Compliance Table
RMS Compliance Statement
Fundamental RMS Requirements Implemented RMS Compliant
Managed Recipe Object ❒ Yes  ❒ No
Recipe Namespace Object ❒ Yes  ❒ No
Recipe Namespace Manager Object ❒ Yes  ❒ No
Distributed Recipe Namespace Object ❒ Yes  ❒ No
Distributed Recipe Namespace Manager Basic
Requirements
❒ Yes  ❒ No
OSS services ❒ Yes  ❒ No
Attachment and detachment ❒ Yes  ❒ No
Attached segment read-only level ❒ Yes  ❒ No
Delete distributed namespace ❒ Yes  ❒ No
Change request management ❒ Yes  ❒ No
Segment change request ❒ Yes  ❒ No
Segment action complete ❒ Yes  ❒ No
Segment action complete ❒ Yes  ❒ No
Get change request ❒ Yes  ❒ No
Rebuild distributed recipe namespace ❒ Yes  ❒ No
...
5
Do not mark YES unless all fundamental RMS requirements are implemented.
15. 6   Distributed  Recipe  Namespace  Segment  ó  Table  15.9  describes  the  requirements  for  compliance  to  the
distributed recipe namespace segment.
Table 15.9  Section References for Distributed Recipe Namespace Segment Capabilities
Capability                                                                          Reference                                                                          Fundamental
Fundamental compliance to managed recipe 15.2
Basic requirements 5.4, 5.11.1 (Table 5.1),
10. 1.2
Y
...
SEMI E42-0704 © SEMI 1995, 2004 107
15. 6.1   Distributed  Recipe  Namespace  Segment  Compliance  Table  ó  Table  15.10  provides  a  statement  of
compliance for the distributed recipe namespace segment.
Table 15.10  Distributed Recipe Namespace Segment Compliance Table
RMS Compliance Statement
Fundamental RMS Requirements Implemented RMS Compliant
Managed Recipe ❒ Yes  ❒ No
Basic Requirements ❒ Yes  ❒ No
OSS Services ❒ Yes  ❒ No
Change Management ❒ Yes  ❒ No
Segment Change Approval ❒ Yes  ❒ No
...
Master Segment ❒ Yes  ❒ No ❒ Yes  ❒ No
6
Do not mark YES unless all fundamental RMS requirements are implemented.
7
Additional capabilities may not be marked RMS compliant unless all fundamental RMS requirements are implemented.

15. 7   Distributed  Recipe  Namespace  Recorder  ó  Table  15.11  describes  the  requirements  for  compliance  to  the
...
OSS services 5.3.1, 10.2.1.1ñ10.2.1.3 Y
...
15. 7.1  Distributed  Recipe  Namespace  Recorder  Compliance  Table  ó  Table  15.12  provides  a  statement  of
compliance for the distributed recipe namespace recorder.
Table 15.12  Distributed Recipe Namespace Recorder Compliance Table
RMS Compliance Statement
Fundamental RMS Requirements Implemented RMS Compliant
Basic Requirements ❒ Yes  ❒ No
OSS Services ❒ Yes  ❒ No
Add Segment Record ❒ Yes  ❒ No
Delete Segment Record ❒ Yes  ❒ No
Add Change Request Record ❒ Yes  ❒ No
Delete Change Request Record ❒ Yes  ❒ No
Get Change Request Record ❒ Yes  ❒ No
...
SEMI E42-0704 © SEMI 1995, 2004 108
15. 8  Execution Recipe Compliance ó This section defines RMS compliance for the execution recipe. An execution
recipe exhibits RMS compliance (1) through its attributes and (2) through its structure when transferred.
...
Change management (ChangedBody) (required if recipe executor can change
recipes)
6. 3.1, Table 6.1 Y
Recipe modification EditedBy                                                                                   Table                                                                                   6.1                                                                                   N
Variable parameters (ExecLinkParam)                                                                      Table                                                                      6.1                                                                      N
Multi-part recipe (LinkList)                                                                                       Table                                                                                       6.1                                                                                       N
Recipe compression (SrcRcpID)                                                                                Table                                                                                6.1                                                                                N
Verification signature (VerificationID) Table 6.1, 11.2.2.2 N

15. 8.1   Execution  Recipe  Compliance  Table  ó  Table  15.14  provides  a  statement  of  compliance  for  the  execution
recipe.

Table 15.14  Execution Recipe Compliance Table
RMS Compliance Statement
Fundamental RMS Requirements Implemented RMS Compliant
Mandatory and required attributes ❒ Yes  ❒ No
Recipe identifier (namespace, class, name, version) ❒ Yes  ❒ No
Preservation of designated downloaded attributes ❒ Yes  ❒ No
Change management ❒ Yes  ❒ No
...
Variable Parameters ❒ Yes  ❒ No ❒ Yes  ❒ No
Multi-Part Recipe ❒ Yes  ❒ No ❒ Yes  ❒ No
Recipe Compression ❒ Yes  ❒ No ❒ Yes  ❒ No
Verification Signature ❒ Yes  ❒ No ❒ Yes  ❒ No
9
Do not mark YES unless all fundamental RMS requirements are implemented.
10
Additional capabilities may not be marked RMS compliant unless all fundamental RMS requirements are implemented.
...
SEMI E42-0704 © SEMI 1995, 2004 109
15. 9  Recipe  Executor  ó  Table  15.15  describes  requirements  for  compliance  for  the  recipe  executor.  The  recipe
executor exhibits RMS compliance through its attributes, behavior, and services.
Table 15.15  Section References for Recipe Executor Capabilities
Capability                                                                           Reference                                                                           Fundamental
Fundamental compliance to execution recipe 15.x                                                  Y
OSS services for execution recipe attributes 11.1 Y
OSS services for recipe executor attributes 11.1 Y
Unique recipe identifier 6.6 Y
Protection of recipe execution area 6.5                                                    Y
Change control 6.6, 11.2.10, 11.2.10.5 Y
Recipe download and vertify (RMEDnLdVer) 11.2.1, 14.3 Y
...
Recipe select (RMESelect) 11.2.7, 11.2.7.1, 11.2.7.6,
11. 2.7.6.1, 14.9
Y
...
Recipe compression 6.6.2, 11.2.2.1, 11.2.10.3,
11. 4
N
Recipe modification 6.6.3, 11.2.10.1, 11.4 N
Save last values 6.6.4, 11.2.10.4 N
SEMI E10 Productive state minimum approval (ProdApprove) 6.7, Table 6.2 N
SEMI E10 Productive state minimum certification (ProdCertify) 6.7, Table 6.2 N
Cycle units (RunCycleUnit)                                                                                         Table                                                                                         6.2                                                                                         N
Variable parameters (RecipeSelectParameters)                                                           Table                                                           6.2                                                           N
Change notification (RMEChange) 14.12 N




SEMI E42-0704 © SEMI 1995, 2004 110
15. 9.1   Recipe  Executor  Compliance  Table  ó  Table  15.16  provides  a  statement  of  compliance  for  the  recipe
executor.
...
Recipe Delegation ❒ Yes  ❒ No ❒ Yes  ❒ No
...
agent-specific   attribute   ó   A   recipe   attribute   that
applies only to a specific agent.
agent-specific  parameter  ó  A  variable  parameter  that
has  been  ìtunedî  for  a  specific  agent  by  modifying  its
original  initial  value  and/or  restrictions  and  placing  the
result   in   the   agent-specific   attribute   AgentSpecific
LinkParam.
agent-specific  recipe  ó  A  recipe  stored  in  the  default
namespace of a specific agent.
agent-specific    dataset    ó    The    recipe    component
containing  the  set  of  agent-specific  attributes  that  have
been set for that recipe for a specific agent.
approve    ó    The    operation    that    sets    a    recipe's
ApprovalLevel
attribute to a non-zero value.
approved   ó   A   recipe   is   approved   whenever   its
ApprovalLevel
attribute is non-zero.
approval  level  ó  (1)  Used  to  reflect  different  degrees
of  recipe  development  and  qualification;  (2)  The  value
of the attribute ApprovalLevel
.
attribute ó Information concerning an object that  may
be used either for internal or external purposes.
attribute length ó The length of an individual attribute,
calculated  as  the  sum  of  the  lengths  of  the  attribute
name  and  the  attribute  value,  exclusive  of  formatting
information.
attribute  length  attribute  ó  An  attribute  that  contains
...
attribute  timestamp  attribute  ó  The  timestamp  of  the
last change to any of a recipe's attributes.
authorized user ó A user who is able to identify him or
...
body ó See recipe body.
centralized  namespace  ó  A  recipe  namespace  that  is
not distributed.
certification-level    ó    The    value    of    the    attribute
Certified
.
certified  ó  (1)  Factory-level  approval  that  a  recipe
...
specific agent.
certify  ó  The  operation  of  changing  the  certification-
level of a recipe for an installation of equipment.
class ó See recipe class.
clear  ó  An  attribute  is  cleared  (reset)  by  setting  its
value to its default value.
component  agent  ó  A  subordinate  agent that  provides
services to a supervisory agent.
copy  ó  Originate  a  recipe  by  the  internal  operation  of
duplicating a recipe and assigning a new identifier.
create ó The operation of entering a recipe body into a
namespace and assigning it a new (unused) identifier.
de-certify ó The operation of clearing the certification
level (Certified
attribute) for a specific agent.
dedicated   namespace  ó  A  recipe  namespace  with
exactly one member.
default    namespace    ó    A    dedicated,    centralized
namespace that is used by standalone equipment and/or
for hardware-specific (agent-specific) recipes.
default   value   ó   A   pre-defined   value.   All   non-
...
delegate  ó  An  agent's  supervisor delegates  a  recipe
...
delete ó A recipe is deleted by removing its identifier,
preventing further access, and freeing its storage.
derived  object  form  ó  An  object  form  of  a  recipe  that
...
ìtokenizedî form and requires less storage.
descriptor ó (1) The length and timestamp attributes of
a recipe's attributes or of one of its components; (2) The
set of all of the individual descriptors of a recipe.



SEMI E42-0704 © SEMI 1995, 2004 112
deselect  ó  The  operation  that  prevents  a  selected
recipe from subsequent execution without its first being
selected again.
distributed  recipe  namespace  ó  A  namespace  that  is
capable  of  using  namespace   segments   provided   by
multiple agents.
download ó To send a recipe to the recipe executor.
edit  ó  An  operation  which  creates  a  new  recipe  body
or changes the body of an existing recipe.
editor  ó  A  service  which  allows  a  user  to  create  or
...
eol  ó  The  ASCII  line-feed  character  (0A
16
)  signifying
the end of a line of text.
empty ó An attribute with a binary, list, or text format
with no content.
end of line character ó See eol.
equipment  ó  An  agent  with  associated  hardware  that
provides, at a minimum, recipe execution services.
event ó A detectable occurrence of interest to a service
user.
exception  ó  An  irregular  event  that  indicates  some
...
main  recipe  ó  (1)  The  recipe,  within  a  set  of  one  or
more  recipes  to  be  linked  together,  which  is  not  a
subrecipe; (2) The starting recipe or ìentry pointî for a
set of recipes.
managed recipe ó A recipe that is stored with a recipe
namespace.



SEMI E42-0704 © SEMI 1995, 2004 113
mandatory attribute ó A required attribute that always
exists and has a non-default value.
modify   variable   parameters   ó   An   operation   that
...
namespace ó A domain within which object identifiers
are unique. Also see recipe namespace.
namespace specifier ó An object specifier applied to a
recipe namespace.
non-numeric parameter ó Any variable parameter that
is not a numeric parameter.
non-shared namespace ó A recipe namespace that has
at most one member.
notification service ó A service that does not expect a
response.
numeric  parameter  ó  Any  variable  parameters  that
...
recipe  ó  (1)  The  pre-planned  and  reusable  portion  of
...
recipe body.
recipe attribute ó Structured information concerning a
recipe within the recipe header.
recipe  body  ó  The  contents  of  the  recipe  containing
...
managed  recipe  descriptor  ó  Consists  of  the  generic
attribute  descriptor  and  the  body  descriptor,  followed
by descriptors for any agent-specific datasets.
recipe  execution  area  ó  Storage  used  for  currently
...
for executing it.
recipe identifier ó Consists of the recipe's class, name,
and version.



SEMI E42-0704 © SEMI 1995, 2004 114
recipe  name  ó  A  user-defined  text  string  used  in  the
recipe identifier.  The  name  corresponds  to  PPID  in
Stream 7 implementations.
recipe namespace ó A logical management domain for
(1) the storage and management of recipes, and (2) the
insurance of the uniqueness of recipe identifiers  within
that domain.
recipe    namespace    manager    (manager)    ó    The
...
recipe  specifier  ó  An  object  specifier  applied  to  a
recipe.
recipe storage area ó A storage area for recipes.
rename  ó  The  operation  of  assigning  a  new  identifier
to a recipe or to a recipe namespace.
request service ó A service that requires a response.
SERVICE  class  ó  An  optional  primary  class  used  for
recipes whose purpose is to maintain, prepare, calibrate,
or test the operation of equipment.
reset ó See clear.
resource  ó  An  owned  entity  that  has  an  active  role  in
factory operations.
select  ó  The  act  of  preparing  a  recipe,  or  a  linked
...
been performed.
service ó A service (or message service) represents a
function  offered  to  a  user  by  a  provider.  A  service  is
one of two types: a request or a notification.
service resource ó A set of services within a particular
...
recipe.
shared namespace ó A namespace with more than one
member.
source form ó A recipe body which consists of one or
...
stored  recipe  attribute  ó  An  attribute  of  a  recipe,  a
...
subclass ó A class of recipes within a larger class.
subrecipe ó Any recipe which is referenced by another
...
cluster controller.
timestamp  ó  (1)  The  notation  of  the  date  and  time  of
...
a text string of the form ìyyyymmddhhmmssccî for the
year yyyy, the month mm, the day dd, the hour hh, the
minutes mm, and the seconds ss.
unlink ó An operation that clears all attributes set by a
link operation.
unprotect   ó   An   operation   that   resets   a   recipe's
ApprovalLevel
attribute. (Set it to zero.)
update  ó  The  operation  of  replacing  the  body  of  an
existing recipe.
upload  ó  To  transfer  a  recipe  body  from  the  recipe
executor.
...
validate  ó  The  action  of  checking  a  recipe  to  ensure
...
SEMI E42-0704 © SEMI 1995, 2004 115
which is placed in the Parameters attribute of the recipe
...
operator.
verification    area    ó    Optional    temporary    storage
provided for unverified downloaded recipes.
verify  ó  The  operation  of  (1)  ensuring  that  a  recipe  is
syntactically   correct,   and   (2)   identifying   variable
parameters and external references.
version ó A text string that is part of a recipe's
identifier.
version  number  ó  A  version  consisting  only  of  the
digits  ì0î  through  ì9î  and  one  decimal  point  character
ì.î that can be translated to a pure number. Only whole
numbers  (with  no  decimal  point)  and  numbers  with  a
decimal  point  that  do  not  begin  or  end  with  a  ì0î  may
be used.
whitespace  ó  In  ASCII,  the  space  (20
16
),  tab  (9
16
),
...
Allow a recipeís parameters to be adjusted within specified limits without
...
equipment refers to the recipeís executor, whereas host
...
R1-3.1    Traceability  ó  A  main  management  objective
...
R1-3.2  Recipe Life Cycle ó The lifetime of a recipe in
...
R1-3.3    Recipe  Editing  ó  Traditionally,  recipes  are
...
providing  a  text
form  of  the  equipment  manufacturerís
...
changes  and  allow  the  hostís  change  control  system  to
operate.
R1-3.4    Recipe  Sharing  ó  A  Fab  may  have  several
...
R1-3.5  Protection and Process Control ó The primary
...
piece  of  equipment  so  that  the  recipe  may  be  ìtunedî
...
such interactions.
R1-3.6      Recipe   Selection   ó   Typical   operation   of
...
order ìprocess jobî. In either case, the specification of a
...
that define the process parameter ìBakeTimeî, its initial value and restrictions. The BAKE;2.3 recipe may use the
...
(see 5.3.1.4) or with a ìSelect Recipeî command from the host or operator.
For  example,  a  recipe  ìBAKE;2.3î  may  define  a  variable  ìBakeTimeî,  while  a  recipe  ìRAMP;1,0î  may  define  a
variable ìRampSetPointî. A parent recipe ìXYZî might contain external references in the text of the source form
such as:
...
where ìTime_Aî and ìTime_Bî were both formally defined within XYZ.
...
and this supersedes any value from a recipe attribute or a ìSelect Recipeî command from the host or operator. In
