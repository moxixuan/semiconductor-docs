...
10. 1 Packing    Pack   the   product   with   sufficient
packing  material  to  prevent  any  effects  from  external
impact, moisture, etc.
10. 2 Package  Label   Note  the  following  items  on
the packaging label:
(1) Product Name
(2) Type Name
...
D-150    Standard   Test   Methods   for   A-C   Loss
...
Thermosetting Laminates for Electrical Testing
...
bonding process.
2  Scope
2. 1   This  standard  is  intended  to  se t  an  appropriate
...
2. 2  Only the physical interfaces fo r the frame cassette
...
conveyors.
4. 3  facial datum plane  a vertica l plane that bisects
...
4. 4  frame cassette  an open stru cture that holds one
or more tape frames.
4. 5  horizontal datum plane  a ho rizontal plane from
...
6. 6   Kinematic  Couplings  (optiona l)   The  physical
...
6. 6.1   The  three  features  that  mate  w ith  the  pins  must
...
6. 8  Robotic Handling Flange (opt ional)  On the top
...
SEMI G77-0699 © SEMI 19996
Table 1  Dimensions for Frame Cassette
SymbolValue SpecifiedDatum Measured FromFeature Measured To
θ‡
45 ± 0.5°either vertical datum planesides of position and orientation notches
r13†2.5 mm
2. 6 minimum
...
x4191.5 ± 0.3 mmbilateral datum planeinside wall of frame cassette
...
sensing pads
x41‡30 ± 1 mmbilateral datum planefront right orientation notch on robotic
handling flange
x42‡50 ± 1 mmbilateral datum planefront left orientation notch on robotic
handling flange
x43‡50 ± 1 mmbilateral datum planerear orientation notch on robotic handling
flange
x44‡53 mm
maximum
bilateral datum planeencroachment of box underneath robotic
handling flange
x45‡65.3 ± 1 mmbilateral datum plane
nearest point of side position and
orientation notches on robotic handling
flange
x46‡71 ± 1 mmbilateral datum planesides of robotic handling flange
x47‡58 mm
minimum
bilateral datum planeend of robotic handling flange front and
rear
x56‡194.75 ± 0.25 mmbilateral datum planeoutside edge of bottom rails
x57187 mm
maximum
bilateral datum planebox sides underneath bottom rails
y1193 ± 0.25 mmfacial datum planefront outside edge of frame cassette
y2193 ± 0.25 mmfacial datum planeback outside bottom edge of frame
cassette
...
y45‡65.3 ± 1 mmfacial datum planenearest point of front and rear position and
orientation notches on robotic handling
flange
y46‡71 ± 1 mmfacial datum planefront and rear edge of robotic handling
flange
y47‡58 mm
minimum
...
3. 2   The  definitions  of  the  terms  “R epeatability”  and
...
3. 5   Bump  placement  on  a  semicon d uctor  device  is
...
method.
5. 2.2   It  should  be  noted  that  accurac y  established  by
...
5. 3  automatic    wafer    prober    —    dev ice    that
automatically  and  repeatedly  aligns  the  die  bonding

SEMI G78-0699 © SEMI 19992
...
meaningful.
6. 5.2   The  same  10×  rule  applies  to  t he  vision  system
...
A1-1.10.1.1  A normalizing step to help cal culate the true wafer-to-wafer variation.
...
1  Purpose
1. 1   This  standard  is  intended  to  pr ovide  a  minimum
common  definition  of  timing  accuracy  specifications
for automatic semiconductor test equipment (ATE).
2  Scope
2. 1   The  scope  of  this  standard  incl udes  all  semicon-
...
5. 1.1  ATE — automatic test equipm ent
5. 1.2  DUT — device under test
5. 1.3  NR — Non-return signal form at
5. 1.4  RTx — return to zero, one or c omplement signal
format.
5. 1.5   SBx  —  surround  by  zero,  one  o r  complement
signal format.
5. 1.6  Z — driver off (high impedanc e)
5. 2  Definitions
...
5. 2.6  reference load B — 50 ohms t o ground
5. 2.7  reference load C —
• 50 ohms to low (for driver z to high and high to z
transitions).
• 50  ohms  to  high  (for  driver  z  to  low  and  low  to  z
transitions).
5. 2.8   strobe  compare  —  monitor  DU T  output  at  a
single time point.
5. 2.9   test  cycle  —  inverse  of  test  pat tern  execution
frequency.
5. 2.10   test  fixturing  errors  —  error  in fluenced  by
...
5. 2.11  window compare — monitor D UT continuously
during a time interval.

SEMI G79-0200 © SEMI 20002
...
1  Purpose
1. 1   This  procedure  will  define  a  st andard  process
...
1. 2   Application  of  this  procedure  w ill  simplify  ATE
...
this document.
2. 2   This  procedure  does  not  includ e  analysis  of  the
...
accuracy.
2. 3     Application    of    this    procedure    c an    reduce
...
the applicability of regulatory limitations prior to use.
3  Limitations
3. 1    The   following   limitations   are   i nherent   to   this
procedure:
3. 1.1  The tolerances of each measur ement used in the
procedure are listed in each test, where appropriate.
3. 1.2  The verification methods do n ot include varying
environmental  conditions,  so  results  may  not  reflect
performance at environmental limits.
3. 1.2.1   Due  to  execution  time  limits,  t he  verification
...
3. 1.3   This  method  uses  only  edge  co mpare  mode  and
non-multiplexed   operation   in   providing   a   minimum
representative assessment of AC timing accuracy.
3. 1.4   This  method  does  not  determin e  the  effects  that
duty cycle variations have on AC timing accuracy.
3. 1.5   Not  being  an  exhaustive  analy sis  this  method
...
3. 1.6   Discretion  is  advised  when  int erpreting  OTA
...
10. 2.1   Level  1  requires  no  external  eq uipment  for
...
.
minSBC20%33%020%H
64*minSBC20%33%153%H
64*minSBC20%33%186%L
64*minSBC20%33%053%L
64*minSBC20%33%086%H
64*minSBC20%33%120%L
...
2*minSBC20%33%120%L
2*minSBC20%33%020%H
63*minSBC20%33%153%H
63*minSBC20%33%186%L
63*minSBC20%33%053%L
63*minSBC20%33%086%H
63*minSBC20%33%120%L
63*minSBC20%33%020%H
.
.
32*minSBC20%33%153%H
32*minSBC20%33%186%L
32*minSBC20%33%053%L
32*minSBC20%33%086%H
32*minSBC20%33%120%L
